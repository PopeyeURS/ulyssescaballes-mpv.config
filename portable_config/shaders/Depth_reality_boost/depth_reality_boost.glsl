 //!HOOK RGB
 //!BIND HOOKED
 //!DESC Depth Reality Boost (Expert Edition v2.0)
 //!WIDTH HOOKED.width
 //!HEIGHT HOOKED.height

 const float base_strength   = 0.85;
 const float radius          = 0.90;   // refined micro-detail radius
 const float warmth          = 0.12;
 const float glow_intensity  = 0.12;
 const float chroma_offset   = 0.45;   // more subtle CA
 const float grain_strength  = 0.014;  // refined filmic grain
 ...
 vec4 hook() {
     ...
     // Bloom (slightly wider radius)
     vec3 bloom = vec3(0.0);
     for (int i = -2; i <= 2; i++) {
         for (int j = -2; j <= 2; j++) {
             vec2 offset = vec2(i, j) * texel * 1.25;
     ...
     graded = mix(
         graded,
         vec3(graded.r * 1.08, graded.g * 0.97, graded.b * 0.94),
         0.35
     );
     ...
     // HDR tone mapping + refined gamma
     graded = filmic_hdr_tonecurve(graded);
     graded = pow(graded, vec3(1.10));
     ...
     // Temporal grain (refined)
     float grain = fract(sin(dot(HOOKED_pos.xy + vec2(frame * 0.5), vec2(15.123, 45.321))) * 12345.6789);
     graded += (grain - 0.5) * grain_strength * 0.8;

     // Color fidelity boost (slightly richer)
     graded = mix(graded, graded * vec3(1.025, 1.015, 1.02), 0.05);

     // Final sharpen mix (slightly stronger)
     vec3 final_output = mix(graded, sharpen, 0.14);
